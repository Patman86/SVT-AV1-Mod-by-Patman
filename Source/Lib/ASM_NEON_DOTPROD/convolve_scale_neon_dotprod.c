...
static inline void convolve_horiz_scale_neon_dotprod(const uint8_t *src, int src_stride, int16_t *dst, int dst_stride,
    ...
    const int32_t horiz_offset = (1 << (bd + FILTER_BITS - 1)) + (1 << (ROUND0_BITS - 1));
    ...
    const int32x4_t horiz_offset_vec = vdupq_n_s32((horiz_offset + dotprod_offset) >> 2);
        ...
        do {
            ...
            do {
                ...
                for (int r = 0; r < 8; r++) {
                    ...
                    int16x8_t d0 = convolve8_8_h(t0, t1, t2, t3, t4, t5, t6, t7, filter, horiz_offset_vec);
...
static inline int16x8_t convolve8_8_h_scale_2(uint8x16_t samples[2], const int8x8_t filters,
    ...
    int8x16_t samples1_128 = vreinterpretq_s8_u8(vsubq_u8(samples[1], vdupq_n_u8(128)));
    ...
    if (w == 4) {
        do {
            ...
            do {
                ...
                int16x4_t d0 = convolve8_4_h_scale_2(s0, filter, horiz_offset_vec, permute_tbl);
                int16x4_t d1 = convolve8_4_h_scale_2(s1, filter, horiz_offset_vec, permute_tbl);
                int16x4_t d2 = convolve8_4_h_scale_2(s2, filter, horiz_offset_vec, permute_tbl);
                int16x4_t d3 = convolve8_4_h_scale_2(s3, filter, horiz_offset_vec, permute_tbl);
        ...
        do {
            ...
            do {
                ...
                int16x8_t d0 = convolve8_8_h_scale_2(s0, filter, horiz_offset_vec, permute_tbl);
                int16x8_t d1 = convolve8_8_h_scale_2(s1, filter, horiz_offset_vec, permute_tbl);
                int16x8_t d2 = convolve8_8_h_scale_2(s2, filter, horiz_offset_vec, permute_tbl);
                int16x8_t d3 = convolve8_8_h_scale_2(s3, filter, horiz_offset_vec, permute_tbl);
...
void svt_av1_convolve_2d_scale_neon_dotprod(const uint8_t *src, int src_stride, uint8_t *dst, int dst_stride, int w,
    ...
    if (x_step_qn != 2 * (1 << SCALE_SUBPEL_BITS)) {
        convolve_horiz_scale_neon_dotprod(src - horiz_offset - vert_offset,
                                          src_stride,
                                          im_block,
                                          im_stride,
                                          w,
