...
#define PRINT_CONFIG(label, fmt, ...) SVT_INFO("SVT [config]: %-52s : " fmt "\n", label, ##__VA_ARGS__)
...
void svt_av1_print_lib_params(SequenceControlSet *scs) {
    ...
    if (config->pass == ENC_FIRST_PASS)
        ...
        PRINT_CONFIG("preset / tune / mini-GOP size / pred struct", "%d / %s%s / %d / %s",
                 config->enc_mode,
                 config->tune == 0       ? "VQ"
                     : config->tune == 1 ? "PSNR"
                         : config->tune == 2 ? "SSIM"
        ...
        if (config->scene_change_detection) {
            if (config->intra_period_length != -1)
                PRINT_CONFIG("scd / max / min GOP size / refresh type", "on / %d / %d / %s",
                ...
                PRINT_CONFIG("scd / max / min GOP size / refresh type", "on / inf / %d / %s",
                         config->min_intra_period_length + 1,
                         config->intra_refresh_type == SVT_AV1_FWDKF_REFRESH   ? "open GOP"
                            : config->intra_refresh_type == SVT_AV1_KF_REFRESH ? "closed GOP"
                                                                               : "unknown");
            ...
            if (config->intra_period_length != -1)
                PRINT_CONFIG("scd / max GOP size / refresh type", "off / %d / %s",
                         config->intra_period_length + 1,
                         config->intra_refresh_type == SVT_AV1_FWDKF_REFRESH   ? "open GOP"
                            : config->intra_refresh_type == SVT_AV1_KF_REFRESH ? "closed GOP"
                                                                               : "unknown");
            else
                PRINT_CONFIG("scd / max GOP size / refresh type", "off / inf / %s",
                         config->intra_refresh_type == SVT_AV1_FWDKF_REFRESH   ? "open GOP"
                            : config->intra_refresh_type == SVT_AV1_KF_REFRESH ? "closed GOP"
                                                                               : "unknown");
        }
        ...
        case SVT_AV1_RC_MODE_CQP_OR_CRF:
            if (config->max_bit_rate)
                ...
                if (!config->enable_variance_boost)
                    PRINT_CONFIG("BRC mode / AQ mode / mode / factor", "%d / %s / %s / %.2f",
                             scs->tpl || scs->static_config.enable_variance_boost ? "rate factor" : "CQP assignment",
        ...
        if (config->rate_control_mode != SVT_AV1_RC_MODE_CBR) {
            if (!config->enable_variance_boost) {
                if (config->rate_control_mode == SVT_AV1_RC_MODE_CQP_OR_CRF && !config->max_bit_rate)
                    ...
                    PRINT_CONFIG("AQ mode / variance boost", "%d / disabled",
                             config->enable_adaptive_quantization);
            } else {
                PRINT_CONFIG("AQ mode / variance boost strength / octile / curve", "%d / %d / %d / %s",
                         config->enable_adaptive_quantization,
        ...
        if (!config->balancing_q_bias) {
            if (config->low_q_taper)
                PRINT_CONFIG("QP scale comp. strength / frame luma bias / Q taper", "%.2f / %d / on",
                         config->qp_scale_compress_strength,
                         config->frame_luma_bias >= config->luminance_qp_bias ? config->frame_luma_bias : config->luminance_qp_bias);
            else
                PRINT_CONFIG("QP scale comp. strength / frame luma bias", "%.2f / %d",
                         config->qp_scale_compress_strength,
                         config->frame_luma_bias >= config->luminance_qp_bias ? config->frame_luma_bias : config->luminance_qp_bias);
        }
        else { // config->balancing_q_bias
            if (config->balancing_noise_level_q_bias == 1.0)
                PRINT_CONFIG("Q bias balancing / luminance", "based / %d",
                         config->balancing_luminance_q_bias);
            else
                PRINT_CONFIG("Q bias balancing / luminance / noise level", "based / %d / %.2f",
                         config->balancing_luminance_q_bias,
                         config->balancing_noise_level_q_bias);

            if (config->balancing_r0_dampening_layer == 1)
                PRINT_CONFIG("balancing r0 based decision", "full");
            else
                PRINT_CONFIG("balancing r0 based decision / r0 dampening layer", "full / >=%d",
                         config->balancing_r0_dampening_layer);
        }

        // Film grain
        if (config->film_grain_denoise_strength != 0)
            PRINT_CONFIG("film grain synth / denoise / adapt. blocksize", "%d / %s / %s",
        ...
        if (!config->chroma_grain)
            PRINT_CONFIG("film grain synth - chroma grain", "disabled");
        if (config->photon_noise_iso > 0) {
            if (config->enable_photon_noise_chroma)
                PRINT_CONFIG("photon noise synth - ISO / chroma noise", "%d / on",
                         config->photon_noise_iso);
            else
                PRINT_CONFIG("photon noise synth ISO", "%d",
                         config->photon_noise_iso);
        }

        // Global
        if (config->high_fidelity_encode_psy_bias) {
            PRINT_CONFIG("high fidelity encode PSY bias", "on");
        }
        else if (config->high_quality_encode_psy_bias) {
            PRINT_CONFIG("high quality encode PSY bias", "on");
        }

        if (config->lineart_psy_bias_easter_egg == 1) {
            PRINT_CONFIG("lineart bias ~ Kumiko ~ / variance base / common thr", "%.0f / %d / %d",
            ...
            PRINT_CONFIG("lineart PSY bias thr mode / skip taper variance thr", "testing / %d",
                     config->lineart_variance_thr >> 1);
        else if (config->lineart_psy_bias >= 1.0)
            PRINT_CONFIG("lineart PSY bias / variance base / common thr", "%.0f / %d / %d",
                     floor(config->lineart_psy_bias),
                     config->lineart_variance_thr,
                     config->lineart_variance_thr >> 1);

        if (config->texture_psy_bias_easter_egg == 1) {
            PRINT_CONFIG("texture PSY bias / variance base / common thr", "%.0f - meow - / %d / %d",
            ...
            PRINT_CONFIG("texture PSY bias / variance base / common thr", "%.0f / %d / %d",
                     floor(config->texture_psy_bias),
                     config->texture_variance_thr,
                     config->texture_variance_thr >> 2);
        if (config->noise_psy_bias >= 1.0)
        ...
        if (config->enable_tf) {
            if (config->enable_tf == 2) {
                if (config->alt_tf_decay && !(config->tune == 0 || config->tune == 3))
                    PRINT_CONFIG("temporal filtering - strength / alt decay", "auto / enabled");
                else
                    PRINT_CONFIG("temporal filtering strength", "auto");
            }
            else { // (config->enable_tf == 1)
                if (config->alt_tf_decay && !(config->tune == 0 || config->tune == 3))
                    PRINT_CONFIG("TF - keyframe / non-keyframe strength / alt decay", "%d / %d / enabled",
                             config->kf_tf_strength,
                             config->tf_strength);
                else
                    PRINT_CONFIG("TF - keyframe / non-keyframe strength", "%d / %d",
                             config->kf_tf_strength,
                             config->tf_strength);
            }
        }
        ...
        if (config->enable_qm) {
            if (config->max_qm_level == 15 && config->max_chroma_qm_level == 15)
                PRINT_CONFIG("quantizer matrix - min luma / min chroma level", "%d / %d",
                         config->min_qm_level,
                         config->min_chroma_qm_level);
            else
                PRINT_CONFIG("QM - min / max luma level / min / max chroma level", "%d / %d / %d / %d",
                         config->min_qm_level,
                         config->max_qm_level,
                         config->min_chroma_qm_level,
        ...
        PRINT_CONFIG("sharpness / noise normalization strength", "%d / %d",
                 config->sharpness,
        ...
        if (config->ac_bias) {
            if (config->texture_ac_bias != config->ac_bias || config->lineart_energy_bias != 1.0 || config->texture_energy_bias != 1.0 || config->satd_bias) {
                PRINT_CONFIG("AC bias - universal / texture bias strength", "%.2f / %.2f",
                         config->ac_bias,
                         config->texture_ac_bias);
                PRINT_CONFIG("AC bias - lineart / texture energy bias strength", "%.2f / %.2f",
                         config->lineart_energy_bias,
                         config->texture_energy_bias);
                if (config->satd_bias)
                    PRINT_CONFIG("AC bias - SATD bias strength", "%.2f",
                             config->satd_bias);
                if (config->tx_bias)
                    PRINT_CONFIG("TX bias", "%s",
                             config->tx_bias == 1
                                 ? "full"
        ...
        // DLF
        if (config->enable_dlf_flag && config->dlf_bias)
            PRINT_CONFIG("DLF bias - DLF sharpness / max / min DLF strength", "%d / %d %d / %d %d",
        ...
        if (config->cdef_level && config->cdef_bias) {
            if (config->cdef_bias_min_cdef[0] == 0 &&
                ...
                if (config->texture_cdef_bias_max_cdef[0] == config->cdef_bias_max_cdef[0] &&
                    ...
                    if (config->filtering_noise_detection)
                        PRINT_CONFIG("filtering noise detection", "%s",
                                 config->filtering_noise_detection == 1 ? "on" :
                                 config->filtering_noise_detection == 2 ? "off" :
                                 config->filtering_noise_detection == 3 ? "on (CDEF only)" :
                                                                          "on (restoration only)");
                    PRINT_CONFIG("CDEF bias - CDEF / texture CDEF max strength", "%d,%d %d,%d / %d,%d %d,%d",
                             config->cdef_bias_max_cdef[0],
                             config->cdef_bias_max_cdef[1],
                             config->cdef_bias_max_cdef[2],
                             config->cdef_bias_max_cdef[3],
                ...
                if (config->filtering_noise_detection)
                    PRINT_CONFIG("filtering noise detection", "%s",
                             ...
                             config->filtering_noise_detection == 3 ? "on (CDEF only)" :
                                                                      "on (restoration only)");
                PRINT_CONFIG("CDEF bias - max / min CDEF strength", "%d,%d %d,%d / %d,%d %d,%d",
                         config->cdef_bias_max_cdef[0],
                         config->cdef_bias_max_cdef[1],
                         config->cdef_bias_max_cdef[2],
                         config->cdef_bias_max_cdef[3],
                ...
                if (config->texture_cdef_bias_max_cdef[0] != config->cdef_bias_max_cdef[0] ||
                    ...
                    config->texture_cdef_bias_min_cdef[2] != config->cdef_bias_min_cdef[2] ||
                    config->texture_cdef_bias_min_cdef[3] != config->cdef_bias_min_cdef[3]) {
                    PRINT_CONFIG("CDEF bias - texture max / min CDEF strength", "%d,%d %d,%d / %d,%d %d,%d",
                             config->texture_cdef_bias_max_cdef[0],
                             config->texture_cdef_bias_max_cdef[1],
                             config->texture_cdef_bias_max_cdef[2],
                             config->texture_cdef_bias_max_cdef[3],
    ...
    PRINT_CONFIG("ME_SEG_W0 / ME_SEG_W1 / ME_SEG_W2 / ME_SEG_W3", "%d / %d / %d / %d",
    ...
    PRINT_CONFIG("ME_SEG_H0 / ME_SEG_H1 / ME_SEG_H2 / ME_SEG_H3", "%d / %d / %d / %d",
