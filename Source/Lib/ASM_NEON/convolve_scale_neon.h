...
#ifndef CONVOLVE_SCALE_NEON_H_
#define CONVOLVE_SCALE_NEON_H_
...
static inline void compound_convolve_vert_scale_8tap_neon(const int16_t *src, int src_stride, uint16_t *dst,
    ...
    // A shim of 1 << (COMPOUND_ROUND1_BITS - 1) enables us to use non-rounding shifts -
    // which are generally faster than rounding shifts on modern CPUs.
    const int32x4_t vert_offset = vdupq_n_s32((1 << offset_bits) + (1 << (COMPOUND_ROUND1_BITS - 1)));
    ...
    if (w == 4) {
        do {
            ...
            const ptrdiff_t filter_offset = SUBPEL_TAPS * ((y_qn & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS);
            const int16x8_t filter        = vld1q_s16(y_filter + filter_offset);

            int16x4_t s0, s1, s2, s3, s4, s5, s6, s7;
            load_s16_4x8(s, src_stride, &s0, &s1, &s2, &s3, &s4, &s5, &s6, &s7);

            int16x4_t d0 = compound_convolve8_4_v(s0, s1, s2, s3, s4, s5, s6, s7, filter, vert_offset);
        ...
        do {
            ...
            do {
                ...
                int16x8_t d0 = compound_convolve8_8_v(s0, s1, s2, s3, s4, s5, s6, s7, filter, vert_offset);
...
static inline void compound_avg_convolve_vert_scale_8tap_neon(const int16_t *src, int src_stride, uint8_t *dst8,
    ...
    // A shim of 1 << (COMPOUND_ROUND1_BITS - 1) enables us to use non-rounding shifts -
    // which are generally faster than rounding shifts on modern CPUs.
    const int32_t vert_offset_bits = (1 << offset_bits) + (1 << (COMPOUND_ROUND1_BITS - 1));
    ...
    if (w == 4) {
        ...
        do {
            ...
            do {
                ...
                uint8x8_t d0_u8 = vqrshrun_n_s16(avg, 2 * FILTER_BITS - ROUND0_BITS - COMPOUND_ROUND1_BITS);
...
static inline void compound_dist_wtd_convolve_vert_scale_8tap_neon(const int16_t *src, int src_stride, uint8_t *dst8,
    ...
    // which are generally faster than rounding shifts on modern CPUs.
    const int32x4_t vert_offset = vdupq_n_s32((1 << offset_bits) + (1 << (COMPOUND_ROUND1_BITS - 1)));
    // For the weighted averaging code path we have to substract round offset and convolve round.
    // The shim of 1 << (2 * FILTER_BITS - ROUND0_BITS - COMPOUND_ROUND1_BITS - 1) enables us to use
    // non-rounding shifts. The additional shift by DIST_PRECISION_BITS is needed in order to merge two
    // shift calculations into one.
    const int32x4_t dist_wtd_offset = vdupq_n_s32(
        (1 << (2 * FILTER_BITS - ROUND0_BITS - COMPOUND_ROUND1_BITS - 1 + DIST_PRECISION_BITS)) -
    ...
    if (w == 4) {
        do {
            ...
            int16x4_t d0_s16 = vshrn_n_s32(dst_wtd_avg,
                                           2 * FILTER_BITS - ROUND0_BITS - COMPOUND_ROUND1_BITS + DIST_PRECISION_BITS);
        ...
        do {
            ...
            do {
                ...
                int32x4_t dst_wtd_avg0 = vmlal_s16(dist_wtd_offset, bck_offset, vget_low_s16(d0));
                int32x4_t dst_wtd_avg1 = vmlal_s16(dist_wtd_offset, bck_offset, vget_high_s16(d0));
                ...
                int16x4_t d0_s16_0 = vshrn_n_s32(
                    dst_wtd_avg0, 2 * FILTER_BITS - ROUND0_BITS - COMPOUND_ROUND1_BITS + DIST_PRECISION_BITS);
                int16x4_t d0_s16_1 = vshrn_n_s32(
                    dst_wtd_avg1, 2 * FILTER_BITS - ROUND0_BITS - COMPOUND_ROUND1_BITS + DIST_PRECISION_BITS);
...
static inline void convolve_vert_scale_8tap_neon(const int16_t *src, int src_stride, uint8_t *dst, int dst_stride,
    ...
    int32x4_t vert_offset = vdupq_n_s32((1 << (round_1 - 1)) - (1 << (offset_bits - 1)));
    ...
    if (w == 4) {
        ...
        do {
            ...
            uint8x8_t d = convolve8_8_v(s0, s1, s2, s3, s4, s5, s6, s7, filter, vert_offset);
        ...
        do {
            ...
            do {
                ...
                load_s16_8x8(s, src_stride, &s0[0], &s1[0], &s2[0], &s3[0], &s4[0], &s5[0], &s6[0], &s7[0]);
                load_s16_8x8(s + 8, src_stride, &s0[1], &s1[1], &s2[1], &s3[1], &s4[1], &s5[1], &s6[1], &s7[1]);

                uint8x8_t d0 = convolve8_8_v(
                    s0[0], s1[0], s2[0], s3[0], s4[0], s5[0], s6[0], s7[0], filter, vert_offset);
                uint8x8_t d1 = convolve8_8_v(
                    s0[1], s1[1], s2[1], s3[1], s4[1], s5[1], s6[1], s7[1], filter, vert_offset);
...
static inline void compound_convolve_vert_scale_6tap_neon(const int16_t *src, int src_stride, uint16_t *dst,
    ...
    if (w == 4) {
        do {
            ...
            int16x4_t d0 = compound_convolve6_4_v(s0, s1, s2, s3, s4, s5, filter, vert_offset);
        ...
        do {
            ...
            do {
                ...
                int16x8_t d0 = compound_convolve6_8_v(s0, s1, s2, s3, s4, s5, filter, vert_offset);
    ...
    if (w == 4) {
        ...
        do {
            ...
            do {
                ...
                load_s16_8x6(s, src_stride, &s0[0], &s1[0], &s2[0], &s3[0], &s4[0], &s5[0]);
                load_s16_8x6(s + 8, src_stride, &s0[1], &s1[1], &s2[1], &s3[1], &s4[1], &s5[1]);

                uint8x8_t d0 = convolve6_8_v(s0[0], s1[0], s2[0], s3[0], s4[0], s5[0], filter, vert_offset);
                uint8x8_t d1 = convolve6_8_v(s0[1], s1[1], s2[1], s3[1], s4[1], s5[1], filter, vert_offset);
