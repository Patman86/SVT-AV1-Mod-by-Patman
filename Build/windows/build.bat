...
if -%1-==-- (
    ...
    set "cmake_eflags=%cmake_eflags% -DENABLE_NASM=ON"
    shift
) else if /I "%1"=="dovi" (
...
:help
    ...
    echo Usage: build.bat [2022^|2019^|2017^|2015^|clean] [release^|debug] [nobuild] [test] [shared^|static] [c-only] [avx512] [dovi] [hdr] [no-apps] [no-dec] [no-enc]
    exit /b 1
goto :EOF
